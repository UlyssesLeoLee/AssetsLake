...
const FILES = {
  ...
  enterpriseGovernanceModelTest: join(
    FRONTEND_ROOT,
    'scripts',
    'enterprise-governance-model.test.mjs',
  ),
  issueAssetAttachContractTest: join(
    FRONTEND_ROOT,
    'scripts',
    'issue-asset-attach-contract.test.mjs',
  ),
  ...
  managementPage: join(
    ...
    'plugin-groups',
    'production',
    'ManagementConsolePage.tsx',
  ),
  planningPage: join(FRONTEND_ROOT, 'src', 'plugin-groups', 'production', 'PlanningPage.tsx'),
  advancedProjectManagementPages: join(
    FRONTEND_ROOT,
    'src',
    'plugin-groups',
    'production',
    'AdvancedProjectManagementPages.tsx',
  ),
  productionPrimitives: join(
    FRONTEND_ROOT,
    'src',
    'plugin-groups',
    'production',
    'ProductionPluginPrimitives.tsx',
  ),
  projectManagementPrimitives: join(
    FRONTEND_ROOT,
    'src',
    'plugin-groups',
    'production',
    'ProjectManagementPluginPrimitives.tsx',
  ),
  ...
  aiControlPage: join(FRONTEND_ROOT, 'src', 'plugin-groups', 'production', 'AiControlPage.tsx'),
  ...
  backendProjectManagementHandler: join(
    REPO_ROOT,
    'backend',
    'src',
    'handlers',
    'project_management_handler.rs',
  ),
  backendProjectManagementModel: join(
    REPO_ROOT,
    'backend',
    'src',
    'models',
  ...
  backendProjectManagementRepository: join(
    REPO_ROOT,
    'backend',
    'src',
    'repositories',
    'project_management_repository.rs',
  ),
  backendProjectManagementService: join(
    REPO_ROOT,
    'backend',
    'src',
    'services',
    'project_management_service.rs',
  ),
  ...
  planningMigration: join(
    REPO_ROOT,
    'database',
    'migrations',
    '003_project_management_planning.sql',
  ),
...
test('management integration contract test', () => {
  ...
  const aiControlPage = readText(FILES.aiControlPage);
  ...
  expectIncludes(
    ...
    'plugin id type includes AI orchestration',
  );
  ...
  expectIncludes(
    ...
    'registry exposes AI orchestration plugin',
  );
  expectIncludes(registry, "id: 'production.data-lake'", 'registry exposes data lake plugin');
  expectMatches(
    registry,
    /apiScopes:\s*\[[^\]]*'langgraph'[^\]]*'ai'[^\]]*'data-lake'[^\]]*\]/,
    'registry scopes include LangGraph, AI, and data lake',
  );
  expectIncludes(
    registry,
    'assertHealthyProductArchitecture',
    'registry validates plugin architecture health',
  );
  expectIncludes(
    registry,
    "id: 'production.board.detail'",
    'registry assigns issue detail to the board plugin',
  );
  expectMatches(
    registry,
  ...
  expectIncludes(
    packageJson,
    '"test:browser-paths": "node --test scripts/browser-paths.test.mjs"',
    'package scripts expose selected browser path gate',
  );
  expectIncludes(
    packageJson,
    '"test:gantt-model": "node --test scripts/gantt-model.test.mjs"',
    'package scripts expose Gantt model unit test',
  );
  expectIncludes(
    packageJson,
    '"test:workflow-automation-model": "node --test scripts/workflow-automation-model.test.mjs"',
    'package scripts expose workflow automation model unit test',
  );
  expectIncludes(
    packageJson,
    '"test:report-analytics-model": "node --test scripts/report-analytics-model.test.mjs"',
    'package scripts expose report analytics model unit test',
  );
  expectIncludes(
    packageJson,
    '"test:calendar-workload-model": "node --test scripts/calendar-workload-model.test.mjs"',
    'package scripts expose calendar workload model unit test',
  );
  expectIncludes(
    packageJson,
    '"test:board-planning-model": "node --test scripts/board-planning-model.test.mjs"',
    'package scripts expose board planning model unit test',
  );
  expectIncludes(
    packageJson,
    '"test:issue-evidence-model": "node --test scripts/issue-evidence-model.test.mjs"',
    'package scripts expose issue evidence model unit test',
  );
  expectIncludes(
    packageJson,
    '"test:enterprise-governance-model": "node --test scripts/enterprise-governance-model.test.mjs"',
    'package scripts expose enterprise governance model unit test',
  );
  expectIncludes(
    packageJson,
    '"test:issue-asset-attach": "node --test scripts/issue-asset-attach-contract.test.mjs"',
    'package scripts expose issue asset attach contract test',
  );
  expectIncludes(
    packageJson,
    'scripts/gantt-model.test.mjs',
    'test:ut includes Gantt model unit test',
  );
  expectIncludes(
    packageJson,
    'scripts/workflow-automation-model.test.mjs',
    'test:ut includes workflow automation model unit test',
  );
  expectIncludes(
    packageJson,
    'scripts/report-analytics-model.test.mjs',
    'test:ut includes report analytics model unit test',
  );
  expectIncludes(
    packageJson,
    'scripts/calendar-workload-model.test.mjs',
    'test:ut includes calendar workload model unit test',
  );
  expectIncludes(
    packageJson,
    'scripts/board-planning-model.test.mjs',
    'test:ut includes board planning model unit test',
  );
  expectIncludes(
    packageJson,
    'scripts/issue-evidence-model.test.mjs',
    'test:ut includes issue evidence model unit test',
  );
  expectIncludes(
    packageJson,
    'scripts/enterprise-governance-model.test.mjs',
    'test:ut includes enterprise governance model unit test',
  );
  expectIncludes(
    packageJson,
    'scripts/issue-asset-attach-contract.test.mjs',
    'test:ut includes issue asset attach contract test',
  );
  expectIncludes(
    packageJson,
    'pnpm run test:browser-paths',
    'test:all includes selected browser path gate',
  );
  expectIncludes(
    packageJson,
    '"test:e2e": "pnpm run test:e2e:run && pnpm run test:e2e:artifacts"',
    'package scripts run Playwright and artifact contract',
  );
  expectIncludes(
    packageJson,
    '"test:e2e:run": "playwright test"',
    'package scripts expose raw Playwright run',
  );
  expectIncludes(
    packageJson,
    '"test:e2e:artifacts": "node --test scripts/playwright-artifacts.test.mjs"',
    'package scripts expose Playwright artifact contract',
  );
  expectIncludes(
    packageJson,
    '"test:e2e:trace": "playwright test --trace on"',
    'package scripts expose explicit Playwright trace run',
  );
  expectIncludes(
    packageJson,
    '"test:e2e:report": "playwright show-report playwright-report"',
    'package scripts expose Playwright report viewer',
  );
  expectIncludes(packageJson, '"test:full":', 'package scripts expose full automatic test gate');
  expectIncludes(
    packageJson,
    '"@playwright/test"',
    'package dev dependencies include Playwright test runner',
  );
  expectIncludes(playwrightConfig, "mode: 'on'", 'Playwright config retains traces for every test');
  expectIncludes(
    playwrightConfig,
    'screenshots: true',
    'Playwright trace captures built-in screenshots',
  );
  expectIncludes(
    playwrightConfig,
    "outputDir: './test-results/playwright'",
    'Playwright config writes trace artifacts to stable output dir',
  );
  expectIncludes(
    playwrightConfig,
    'timeout: 120_000',
    'Playwright config tolerates cold Next route compilation',
  );
  expectIncludes(
    playwrightConfig,
    'navigationTimeout: 60_000',
    'Playwright config tolerates cold navigation compilation',
  );
  expectIncludes(playwrightConfig, "'html'", 'Playwright config enables HTML report');
  expectIncludes(
    playwrightConfig,
    'webServer',
    'Playwright config starts the Next app automatically',
  );
  expectIncludes(
    playwrightSpec,
    "test('clicks project management plugin journeys'",
    'Playwright suite clicks project management journeys',
  );
  expectIncludes(
    playwrightSpec,
    "test('clicks Gantt, calendar, workflow, automation, reports, enterprise, and asset library journeys'",
    'Playwright suite clicks product surface journeys',
  );
  expectIncludes(
    playwrightSpec,
    "test('uploads an asset through the browser flow'",
    'Playwright suite clicks upload journey',
  );
  expectIncludes(
    playwrightSpec,
    'setInputFiles',
    'Playwright upload journey sets a browser file input',
  );
  expectIncludes(
    playwrightMocks,
    "page.route('**/api/**'",
    'Playwright suite mocks API routes for deterministic browser traces',
  );
  expectIncludes(
    playwrightMocks,
    '/api/project-management/gantt',
    'Playwright API mock covers Gantt product endpoint',
  );
  expectIncludes(
    playwrightMocks,
    '/api/assets/upload',
    'Playwright API mock covers upload endpoint',
  );
  expectIncludes(
    playwrightArtifactsTest,
    "test('playwright trace and report artifacts are retained'",
    'Playwright artifact contract declares trace/report retention test',
  );
  expectIncludes(
    playwrightArtifactsTest,
    'playwright-results.json',
    'Playwright artifact contract reads JSON test results',
  );
  expectIncludes(
    playwrightArtifactsTest,
    "attachment.name === 'trace' && attachment.contentType === 'application/zip'",
    'Playwright artifact contract validates trace zip attachments',
  );
  expectIncludes(
    playwrightArtifactsTest,
    'playwright-report',
    'Playwright artifact contract validates HTML report output',
  );
  expectIncludes(playwrightDocs, 'Trace Retention', 'Playwright docs explain trace retention');
  expectIncludes(
    playwrightDocs,
    'frontend/test-results/playwright/',
    'Playwright docs point to trace output directory',
  );
  expectIncludes(
    browserPathsTest,
    "test('selected browser path product journey gate'",
    'browser path test declares product journey gate',
  );
  expectIncludes(
    browserPathsTest,
    "pathname: '/issues/browser-path-issue'",
    'browser path test covers issue detail journey',
  );
  expectIncludes(
    browserPathsTest,
    "pathname: '/enterprise'",
    'browser path test covers enterprise admin journey',
  );
  expectIncludes(
    browserPathsTest,
    'useEnterpriseControls\\(DEFAULT_PROJECT_ID\\)',
    'browser path test verifies enterprise data hook',
  );
  expectIncludes(
    browserPathsTest,
    'useProjectGantt\\(DEFAULT_PROJECT_ID\\)',
    'browser path test verifies Gantt data hook',
  );
  expectIncludes(
    nextConfig,
    'keepManagedPath',
    'Next config filters empty optional SWC managed paths',
  );
  expectIncludes(
    nextConfig,
    'infrastructureLogging',
    'Next config controls webpack infrastructure logging',
  );
  expectIncludes(
    nextConfig,
    "level: 'error'",
    'Next config suppresses false-positive webpack cache warnings',
  );
  expectIncludes(
    ganttModelTest,
    "test('gantt timeline model lays out blocking path'",
    'Gantt model unit test covers blocking path layout',
  );
  expectIncludes(
    ganttModelTest,
    "scheduleRisk, 'blocked'",
    'Gantt model unit test asserts blocked risk',
  );
  expectIncludes(
    workflowAutomationModelTest,
    "test('workflow automation model builds guarded workflow and LangGraph execution plan'",
    'workflow automation model unit test covers guarded workflow and execution plan',
  );
  expectIncludes(
    workflowAutomationModelTest,
    'graphCoveragePercent',
    'workflow automation model unit test asserts LangGraph coverage',
  );
  expectIncludes(
    reportAnalyticsModelTest,
    "test('report analytics model builds dashboard KPIs, flow, SLA, and readiness'",
    'report analytics model unit test covers report dashboard metrics',
  );
  expectIncludes(
    reportAnalyticsModelTest,
    'readinessStatus',
    'report analytics model unit test asserts readiness status',
  );
  expectIncludes(
    calendarWorkloadModelTest,
    "test('calendar workload model sorts events and derives lane risk'",
    'calendar workload model unit test covers sorting and lane risk',
  );
  expectIncludes(
    calendarWorkloadModelTest,
    'buildCalendarWorkloadModel',
    'calendar workload model unit test asserts workload model output',
  );
  expectIncludes(
    boardPlanningModelTest,
    "test('board planning model derives WIP, swimlanes, and risk queue'",
    'board planning model unit test covers WIP, swimlanes, and risk queue',
  );
  expectIncludes(
    boardPlanningModelTest,
    'buildBoardPlanningModel',
    'board planning model unit test asserts board model output',
  );
  expectIncludes(
    issueEvidenceModelTest,
    "test('issue evidence model derives readiness, data lake links, and LangGraph suggestions'",
    'issue evidence model unit test covers data lake and LangGraph evidence',
  );
  expectIncludes(
    issueEvidenceModelTest,
    'buildIssueEvidenceModel',
    'issue evidence model unit test asserts issue evidence output',
  );
  expectIncludes(
    enterpriseGovernanceModelTest,
    "test('enterprise governance model scores permissions, gates, webhooks, and audit'",
    'enterprise governance model unit test covers permissions, gates, webhooks, and audit',
  );
  expectIncludes(
    enterpriseGovernanceModelTest,
    'buildEnterpriseGovernanceModel',
    'enterprise governance model unit test asserts governance output',
  );
  expectIncludes(
    issueAssetAttachContractTest,
    "test('issue asset attach contract keeps API, hook, page, and E2E aligned'",
    'issue asset attach contract covers API, hook, page, and E2E wiring',
  );
  ...
  expectMatches(
    ganttRoute,
    /PluginRouteHost[\s\S]*expectedRouteId="production\.gantt"/,
    'Next App Router /gantt adapter is plugin-hosted',
  );
  expectMatches(
    calendarRoute,
    /PluginRouteHost[\s\S]*expectedRouteId="production\.calendar"/,
    'Next App Router /calendar adapter is plugin-hosted',
  );
  expectMatches(
    reportsRoute,
    /PluginRouteHost[\s\S]*expectedRouteId="production\.reports"/,
    'Next App Router /reports adapter is plugin-hosted',
  );
  expectMatches(
    workflowRoute,
    /PluginRouteHost[\s\S]*expectedRouteId="production\.workflow"/,
    'Next App Router /workflow adapter is plugin-hosted',
  );
  expectMatches(
    automationRoute,
    /PluginRouteHost[\s\S]*expectedRouteId="production\.automation"/,
    'Next App Router /automation adapter is plugin-hosted',
  );
  expectMatches(
    enterpriseRoute,
    /PluginRouteHost[\s\S]*expectedRouteId="production\.enterprise"/,
    'Next App Router /enterprise adapter is plugin-hosted',
  );
  expectIncludes(
    productionIndex,
  ...
  for (const pageExport of [
    ...
    expectIncludes(
      productionIndex,
      `export { ${pageExport} }`,
      `production plugin group exports ${pageExport}`,
    );
  }
  ...
  expectIncludes(
    ...
    'frontend types expose management intelligence',
  );
  expectIncludes(
    productionTypes,
    'langgraph_nodes: LangGraphNode[];',
    'frontend types include LangGraph nodes',
  );
  expectIncludes(
    productionTypes,
    'data_lake_feeds: DataLakeFeed[];',
    'frontend types include data lake feeds',
  );
  expectIncludes(
    productionTypes,
    'automation_rules: AiAutomationRule[];',
    'frontend types include AI automation rules',
  );
  expectIncludes(
    productionTypes,
    'export interface AiAutopilotPlanRequest',
    'frontend types expose AI Autopilot request',
  );
  expectIncludes(
    productionTypes,
    'export interface AiAutopilotPlanResponse',
    'frontend types expose AI Autopilot response',
  );
  expectIncludes(
    productionTypes,
    'export interface AiAutopilotDecisionReview',
    'frontend types expose AI Autopilot decision review',
  );
  expectIncludes(
    productionTypes,
    'decision_review: AiAutopilotDecisionReview',
    'frontend Autopilot response carries decision review',
  );
  expectMatches(
    productionTypes,
    /IssueWorkLog[\s\S]*CreateIssueWorkLogRequest/,
    'frontend production types include issue work logs',
  );
  expectMatches(
    productionTypes,
    /story_points[\s\S]*rank_key/,
    'frontend production types include issue planning fields',
  );
  expectMatches(
    projectManagementTypes,
    /ProjectManagementEpic[\s\S]*ProjectManagementSprint[\s\S]*IssueDependency[\s\S]*IssueEvent/,
    'project management types cover Phase 1 planning contracts',
  );
  expectMatches(
    projectManagementTypes,
    /ProjectGanttSnapshot[\s\S]*ProjectCalendarSnapshot[\s\S]*ProjectReportsSnapshot[\s\S]*ProjectWorkflowCatalog[\s\S]*ProjectAutomationCatalog[\s\S]*ProjectEnterpriseControls/,
    'project management types cover Phase 2-5 product payloads',
  );
  expectMatches(
    projectManagementTypes,
    /ProjectGanttItem[\s\S]*priority: IssuePriority[\s\S]*start_date\?: string[\s\S]*story_points\?: number/,
    'project management types carry Gantt schedule inputs',
  );
  expectMatches(
    projectManagementTypes,
    /ProjectCalendarEvent[\s\S]*ProjectCalendarLane[\s\S]*ProjectCalendarWorkloadDay[\s\S]*lanes: ProjectCalendarLane\[][\s\S]*workload: ProjectCalendarWorkloadDay\[]/,
    'project management types cover structured calendar lanes and workload contracts',
  );
  expectMatches(
    projectManagementTypes,
    /ProjectBurndownPoint[\s\S]*ProjectVelocityPoint[\s\S]*ProjectCumulativeFlowPoint[\s\S]*ProjectCycleTimeMetric[\s\S]*ProjectSlaMetric/,
    'project management types cover structured report analytics contracts',
  );
  expectMatches(
    projectManagementTypes,
    /ProjectWorkflowTransition[\s\S]*ProjectWorkflowApprovalPolicy[\s\S]*ProjectAutomationRule[\s\S]*ProjectAutomationRunbookStep/,
    'project management types cover structured workflow and automation contracts',
  );
  expectMatches(
    projectManagementTypes,
  ...
  expectIncludes(
    productionApi,
    ` '${CONTRACT_ENDPOINT}'`.trim(),
    'frontend API client targets management endpoint',
  );
  expectIncludes(
    productionApi,
    'management: {',
    'frontend API client exposes management namespace',
  );
  expectIncludes(
    productionApi,
    '/api/management/autopilot-plan',
    'frontend API client targets AI Autopilot plan endpoint',
  );
  expectMatches(
    productionApi,
    /\/api\/issues\/\$\{id\}\/comments[\s\S]*\/api\/issues\/\$\{id\}\/work-logs[\s\S]*apiClient\.delete\(`\/api\/issues\/\$\{id\}`\)/,
    'frontend production API covers issue comments, work logs, and delete',
  );
  expectMatches(
    ...
    'project management API declares planning and productization endpoints',
  );
  expectMatches(
    projectManagementApi,
    /Promise<ProjectGanttSnapshot>[\s\S]*Promise<ProjectCalendarSnapshot>[\s\S]*Promise<ProjectReportsSnapshot>[\s\S]*Promise<ProjectWorkflowCatalog>[\s\S]*Promise<ProjectAutomationCatalog>[\s\S]*Promise<ProjectEnterpriseControls>/,
    'project management API has typed Phase 2-5 responses',
  );
  expectIncludes(
    productionHooks,
    'export function useManagementIntelligence()',
    'frontend hook exposes management intelligence',
  );
  expectIncludes(
    productionHooks,
    "queryKey: ['management-intelligence']",
    'frontend hook uses stable integration query key',
  );
  expectMatches(
    productionHooks,
    /useIssueComments[\s\S]*useIssueWorkLogs[\s\S]*useUpdateIssue[\s\S]*useDeleteIssue[\s\S]*useAddIssueComment[\s\S]*useCreateIssueWorkLog[\s\S]*useAttachIssueAsset/,
    'frontend hooks expose issue core loop mutations, asset attach, and queries',
  );
  expectMatches(
    projectManagementHooks,
    /useProjectManagementPlan[\s\S]*useProjectManagementEpics[\s\S]*useProjectManagementSprints[\s\S]*useIssueDependencies/,
    'project management hooks expose planning query keys',
  );

  expectIncludes(
    managementPage,
    'useManagementIntelligence',
    'management page reads intelligence hook',
  );
  expectIncludes(
    managementPage,
    'intelligence?.langgraph_nodes',
    'management page consumes LangGraph nodes',
  );
  expectIncludes(
    managementPage,
    'intelligence?.data_lake_feeds',
    'management page consumes data lake feeds',
  );
  expectIncludes(
    managementPage,
    'intelligence?.automation_rules',
    'management page consumes AI automation rules',
  );
  expectIncludes(
    managementPage,
    'useIssues({ page_size: 250 })',
    'management page integrates production issues',
  );
  expectIncludes(
    managementPage,
    "useAssets({ status: 'active', page_size: 100 })",
    'management page integrates data lake assets',
  );
  expectIncludes(
    managementPage,
    'useMilestones(DEFAULT_PROJECT_ID)',
    'management page integrates roadmap milestones',
  );
  expectIncludes(
    planningPage,
    'useIssues({ page_size: 250 })',
    'planning page integrates issue backlog',
  );
  expectIncludes(
    planningPage,
    'useMilestones(DEFAULT_PROJECT_ID)',
    'planning page integrates milestone anchors',
  );
  expectIncludes(planningPage, 'buildPlanningModel', 'planning page builds a local planning model');
  expectMatches(
    issueDetailPage,
    /useAttachIssueAsset[\s\S]*buildIssueEvidenceModel[\s\S]*Data Lake Asset ID[\s\S]*Attach Evidence[\s\S]*IssueEvidenceReadinessPanel[\s\S]*DataLakeEvidencePanel[\s\S]*LangGraphRecommendationPanel/,
    'Issue detail renders data lake evidence, LangGraph recommendation panels, and asset attach controls',
  );
  expectMatches(
    advancedProjectManagementPages,
    /export \{ GanttPage \}[\s\S]*export \{ ReportsPage \}/,
    'legacy advanced page module is only a compatibility barrel',
  );
  expectIncludes(
    productionPrimitives,
    'export function IssueCard',
    'production primitives own shared issue card UI',
  );
  expectIncludes(
    projectManagementPrimitives,
    'export function usePlanIssues',
    'project management primitives own shared planning query composition',
  );
  for (const [pageName, source] of Object.entries(atomicAdvancedPages)) {
    expectMatches(
      source,
      new RegExp(`export function ${pageName}`),
      `${pageName} has an atomic page implementation`,
    );
    expectMatches(
      source,
      /ProjectManagementPluginPrimitives/,
      `${pageName} consumes shared project management primitives`,
    );
  }
  expectIncludes(
    atomicAdvancedPages.GanttPage,
    'useProjectGantt(DEFAULT_PROJECT_ID)',
    'Gantt page consumes product Gantt hook',
  );
  expectMatches(
    atomicAdvancedPages.GanttPage,
    /buildGanttTimelineModel[\s\S]*Schedule Timeline[\s\S]*Critical Path[\s\S]*Dependency Map[\s\S]*Risk Queue/,
    'Gantt page renders timeline model, critical path, dependency map, and risk queue',
  );
  expectMatches(
    boardPage,
    /buildBoardPlanningModel[\s\S]*BoardHealthPanel[\s\S]*BoardSwimlanePanel[\s\S]*BoardRiskQueuePanel/,
    'Kanban board renders WIP health, swimlanes, and risk queue panels',
  );
  expectMatches(
    boardPage,
    /BoardAiAssistPanel[\s\S]*productionApi\.management\.autopilotPlan[\s\S]*buildBoardAiPlanRequest/,
    'Kanban board exposes page-level AI assisted planning through the Autopilot contract',
  );
  expectMatches(
    aiControlPage,
    /DecisionReviewPanel[\s\S]*LangGraphChain[\s\S]*toDecisionReview/,
    'AI Control renders decision review and LangGraph execution chain',
  );
  expectMatches(
    ganttModel,
    /normalizeBlockingEdge[\s\S]*findCriticalPath[\s\S]*getScheduleRisk[\s\S]*buildGanttTimelineModel/,
    'Gantt model owns layout, critical path, blocking edge, and schedule risk logic',
  );
  expectIncludes(
    atomicAdvancedPages.CalendarPage,
    'useProjectCalendar(DEFAULT_PROJECT_ID)',
    'Calendar page consumes product calendar hook',
  );
  expectMatches(
    atomicAdvancedPages.CalendarPage,
    /buildCalendarWorkloadModel[\s\S]*Upcoming Work[\s\S]*CalendarLanePanel[\s\S]*CalendarWorkloadPanel[\s\S]*CalendarRiskPanel/,
    'Calendar page renders structured workload, lane, and risk panels',
  );
  expectIncludes(
    atomicAdvancedPages.ReportsPage,
    'useProjectReports(DEFAULT_PROJECT_ID)',
    'Reports page consumes product reports hook',
  );
  expectMatches(
    atomicAdvancedPages.ReportsPage,
    /buildReportsDashboardModel[\s\S]*BurndownTrendPanel[\s\S]*VelocityPanel[\s\S]*CycleSlaPanel[\s\S]*DeliveryReadinessPanel/,
    'Reports page renders structured analytics dashboard panels',
  );
  expectIncludes(
    atomicAdvancedPages.WorkflowPage,
    'useProjectWorkflow(DEFAULT_PROJECT_ID)',
    'Workflow page consumes product workflow hook',
  );
  expectIncludes(
    atomicAdvancedPages.AutomationPage,
    'useProjectAutomation(DEFAULT_PROJECT_ID)',
    'Automation page consumes product automation hook',
  );
  expectMatches(
    atomicAdvancedPages.WorkflowPage,
    /buildWorkflowDesignerModel[\s\S]*WorkflowPolicyPanel[\s\S]*WorkflowTransitionDesigner/,
    'Workflow page renders structured workflow designer and policy panels',
  );
  expectMatches(
    atomicAdvancedPages.AutomationPage,
    /buildAutomationExecutionPlan[\s\S]*AutomationRuleCards[\s\S]*AutomationExecutionPlanPanel/,
    'Automation page renders structured automation rules and execution plan',
  );
  expectMatches(
    workflowAutomationModel,
    /buildWorkflowDesignerModel[\s\S]*resolveWorkflowTransitions[\s\S]*buildAutomationExecutionPlan[\s\S]*ruleReadsDataLake/,
    'workflow automation model owns transition coverage and LangGraph execution logic',
  );
  expectMatches(
    reportAnalyticsModel,
    /buildReportsDashboardModel[\s\S]*buildBurndownPoints[\s\S]*latestFlowPoint[\s\S]*buildReadinessStatus/,
    'report analytics model owns KPI, burndown, flow, SLA, and readiness logic',
  );
  expectMatches(
    calendarWorkloadModel,
    /sortEventsByDate[\s\S]*buildLaneSummaries[\s\S]*buildCalendarWorkloadModel/,
    'calendar workload model owns event sorting, lane risk, and workload assembly logic',
  );
  expectMatches(
    boardPlanningModel,
    /getBoardRisk[\s\S]*buildBoardColumns[\s\S]*buildBoardSwimlanes[\s\S]*buildBoardPlanningModel/,
    'board planning model owns WIP, swimlane, and risk queue logic',
  );
  expectMatches(
    issueEvidenceModel,
    /deriveEvidenceRisk[\s\S]*buildEvidenceChecklist[\s\S]*buildDataLakeEvidenceLinks[\s\S]*buildLangGraphSuggestions[\s\S]*buildIssueEvidenceModel/,
    'issue evidence model owns readiness, data lake links, and LangGraph suggestions',
  );
  expectIncludes(
    atomicAdvancedPages.EnterpriseAdminPage,
    'useEnterpriseControls(DEFAULT_PROJECT_ID)',
    'Enterprise page consumes enterprise controls hook',
  );
  expectMatches(
    atomicAdvancedPages.EnterpriseAdminPage,
    /buildEnterpriseGovernanceModel[\s\S]*GovernanceReadinessPanel[\s\S]*GovernanceRiskPanel[\s\S]*roles\.map[\s\S]*permissions\.map[\s\S]*notifications\.map[\s\S]*webhooks\.map[\s\S]*templates\.map[\s\S]*ciGates\.map/,
    'Enterprise page renders governance model plus structured role, notification, webhook, template, and CI controls',
  );
  expectMatches(
    enterpriseGovernanceModel,
    /buildPermissionCoverage[\s\S]*buildGovernanceRisks[\s\S]*buildEnterpriseGovernanceModel/,
    'enterprise governance model owns permissions, risk, and readiness scoring logic',
  );

  expectIncludes(
    backendHandlersMod,
    'pub mod management_handler;',
    'backend handler module exports management handler',
  );
  expectIncludes(
    backendHandlersMod,
    'pub mod project_management_handler;',
    'backend handler module exports project management handler',
  );
  expectIncludes(
    backendModelsMod,
    'pub mod project_management;',
    'backend model module exports project management model',
  );
  expectIncludes(
    backendRepositoriesMod,
    'pub mod project_management_repository;',
    'backend repository module exports project management repository',
  );
  expectIncludes(
    backendServicesMod,
    'pub mod project_management_service;',
    'backend service module exports project management service',
  );
  expectIncludes(backendRoutes, 'management_handler', 'backend routes import management handler');
  expectIncludes(
    backendRoutes,
    'project_management_handler',
    'backend routes import project management handler',
  );
  expectIncludes(
    backendRoutes,
  ...
  expectMatches(
    backendHandler,
    /struct ManagementIntelligenceResponse[\s\S]*langgraph_nodes[\s\S]*data_lake_feeds[\s\S]*automation_rules/,
    'backend response contract includes all intelligence channels',
  );
  expectMatches(
    backendHandler,
    /LangGraphNode[\s\S]*DataLakeFeed[\s\S]*AiAutomationRule/,
    'backend contract models LangGraph, data lake, and AI automation',
  );
  expectIncludes(
    backendHandler,
    'ApiResponse::ok(response)',
    'backend wraps management intelligence in standard API response',
  );
  expectIncludes(
    backendHandler,
    '#[post("/api/management/autopilot-plan")]',
    'backend exposes AI Autopilot plan endpoint',
  );
  expectMatches(
    backendHandler,
    /AiAutopilotPlanRequest[\s\S]*AiAutopilotPlanResponse[\s\S]*build_ai_autopilot_plan/,
    'backend contracts structured AI Autopilot planning',
  );
  expectMatches(
    backendHandler,
    /AiAutopilotDecisionReview[\s\S]*build_decision_review[\s\S]*normalize_decision_review/,
    'backend Autopilot contract normalizes structured decision review',
  );
  expectMatches(
    backendProjectManagementHandler,
    /#\[get\("\/api\/project-management\/plan"\)\][\s\S]*#\[get\("\/api\/project-management\/epics"\)\][\s\S]*#\[post\("\/api\/project-management\/epics"\)\][\s\S]*#\[get\("\/api\/project-management\/sprints"\)\][\s\S]*#\[post\("\/api\/project-management\/sprints"\)\][\s\S]*#\[get\("\/api\/project-management\/dependencies"\)\][\s\S]*#\[post\("\/api\/project-management\/dependencies"\)\][\s\S]*#\[get\("\/api\/project-management\/events"\)\]/,
    'backend project management handler exposes planning endpoints',
  );
  expectMatches(
    backendProjectManagementRepository,
    /i\.epic_id[\s\S]*i\.sprint_id[\s\S]*i\.story_points::DOUBLE PRECISION[\s\S]*i\.rank_key/,
    'backend project planning summaries include sprint, epic, story points, and rank',
  );
  expectMatches(
    backendProjectManagementHandler,
    /project-management\/gantt/,
    'backend project management handler remains loaded after issue core additions',
  );
  expectMatches(
    backendProjectManagementService,
    /gantt_snapshot[\s\S]*calendar_snapshot/,
    'backend project management service remains loaded after issue core additions',
  );
  expectMatches(
    backendProjectManagementService,
    /"priority": &issue\.priority[\s\S]*"start_date": issue\.start_date[\s\S]*"story_points": issue\.story_points/,
    'backend Gantt snapshot exposes schedule input fields',
  );
  expectMatches(
    backendRoutes,
  ...
  expectMatches(
    backendProjectManagementModel,
    /ProjectManagementEpic[\s\S]*ProjectManagementSprint/,
    'backend project management model remains available for issue planning fields',
  );
  expectMatches(
    backendProjectManagementHandler,
    /#\[get\("\/api\/project-management\/gantt"\)\][\s\S]*#\[get\("\/api\/project-management\/calendar"\)\][\s\S]*#\[get\("\/api\/project-management\/reports"\)\][\s\S]*#\[get\("\/api\/project-management\/workflow"\)\][\s\S]*#\[get\("\/api\/project-management\/automation"\)\][\s\S]*#\[get\("\/api\/project-management\/enterprise"\)\]/,
    'backend project management handler exposes Phase 2-5 endpoints',
  );
  expectMatches(
    backendProjectManagementModel,
    /SprintStatus[\s\S]*IssueDependencyType[\s\S]*IssueEventType[\s\S]*ProjectManagementEpic[\s\S]*ProjectManagementSprint[\s\S]*ProjectManagementPlan/,
    'backend project management model mirrors frontend planning contracts',
  );
  expectMatches(
    backendProjectManagementRepository,
    ...
    'backend project management repository supports Phase 1 operations',
  );
  expectMatches(
    backendProjectManagementService,
    /planning_plan[\s\S]*backlog[\s\S]*active_sprint[\s\S]*dependencies[\s\S]*recent_events/,
    'backend project management service assembles planning plan payload',
  );
  expectMatches(
    backendProjectManagementService,
    /gantt_snapshot[\s\S]*calendar_snapshot[\s\S]*reports_snapshot[\s\S]*workflow_catalog[\s\S]*automation_catalog[\s\S]*enterprise_controls/,
    'backend project management service assembles Phase 2-5 payloads',
  );
  expectMatches(
    backendProjectManagementService,
    /"burndown": \{[\s\S]*"points": \[[\s\S]*"velocity": \{[\s\S]*"predictability_percent"[\s\S]*"cycle_time": \{[\s\S]*"sla": \{/,
    'backend report payload exposes structured analytics metrics',
  );
  expectMatches(
    backendProjectManagementService,
    /"lanes": \[[\s\S]*"workload": workload[\s\S]*"review_calendar": "ready"/,
    'backend calendar payload exposes structured lanes and workload metrics',
  );
  expectMatches(
    backendProjectManagementService,
    /"transitions": \[[\s\S]*"approval_policy": \{[\s\S]*"rules": \[[\s\S]*"runbook": \[/,
    'backend workflow and automation payloads expose structured transition and LangGraph catalogs',
  );
  expectMatches(
    backendProjectManagementService,
    /"roles": \[[\s\S]*"permissions"[\s\S]*"notifications": \[[\s\S]*"delivery_policy"[\s\S]*"import_export": \[[\s\S]*"webhooks": \[[\s\S]*"templates": \[[\s\S]*"ci_gates": \[[\s\S]*"audit": \{/,
    'backend enterprise controls expose structured Phase 5 productization catalog',
  );
  expectMatches(
    backendMain,
    /project_management_service::ProjectManagementService[\s\S]*pub project_management_service: ProjectManagementService[\s\S]*project_management_service,/,
    'backend AppState wires project management service',
  );
  expectMatches(
    planningMigration,
    ...
    'planning migration adds epics, sprints, dependencies, and events',
  );
  expectMatches(
    planningMigration,
    /ALTER TABLE issues ADD COLUMN IF NOT EXISTS epic_id[\s\S]*ALTER TABLE issues ADD COLUMN IF NOT EXISTS sprint_id[\s\S]*ALTER TABLE issues ADD COLUMN IF NOT EXISTS story_points[\s\S]*ALTER TABLE issues ADD COLUMN IF NOT EXISTS rank_key/,
    'planning migration extends issues for sprint planning and ranking',
  );
  expectMatches(
    productizationMigration,
    ...
    'productization migration adds Phase 2-5 tables',
  );
  expectMatches(
    issueCoreMigration,
    ...
    'issue core migration adds work logs and planning fields',
  );
});
