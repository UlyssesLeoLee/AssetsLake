...
export function WorkflowPage() {
  ...
  const workflowModel = buildWorkflowDesignerModel(
    workflow,
    Array.from(WORKFLOW_STEPS) as IssueStatus[],
  );

  return (
    <PageShell
      title="Workflow"
      subtitle="Configurable statuses, transitions, validators, guards, and approval policy"
    >
      <div className="grid gap-4 md:grid-cols-4">
        <Metric label="Statuses" value={workflowModel.statuses.length} detail="workflow states" />
        <Metric
          label="Transitions"
          value={workflowModel.transitions.length}
          detail="configured edges"
          tone="text-cyan-300"
        />
        <Metric
          label="Evidence Gates"
          value={workflowModel.evidenceGateCount}
          detail="data lake checks"
          tone="text-emerald-300"
        />
        <Metric
          label="Approval Gates"
          value={workflowModel.approvalGateCount}
          detail="human review"
          tone="text-amber-300"
        />
      </div>
