...
export function CalendarPage() {
  ...
  return (
    <PageShell title="Calendar" subtitle="Sprint, release, review, vendor delivery, and due-date planning">
      <div className="grid gap-4 md:grid-cols-4">
        <Metric label="Events" value={calendarModel.eventCount} detail="dated issue events" />
        <Metric label="Risk" value={calendarModel.riskCount} detail="overdue, blocked, or missing evidence" tone="text-amber-300" />
        <Metric
          label="Next Date"
          ...
          tone="text-cyan-300"
        />
        <Metric label="Undated" value={calendarModel.undatedCount} detail="needs planning" tone="text-rose-300" />
      </div>
        ...
        <Panel title="Upcoming Work" icon={<Calendar className="h-4 w-4" />}>
          <div className="divide-y divide-surface-border">
              ...
              <Link key={event.id} href={`/issues/${event.id}`} className="block py-3 transition hover:bg-slate-800/40">
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <div className="text-xs font-medium text-brand-300">{event.lane} / {event.item_type}</div>
                    <div className="mt-1 truncate text-sm font-medium text-slate-100">{event.title}</div>
                    {event.owner && <div className="mt-1 truncate text-xs text-slate-500">{event.owner}</div>}
                  </div>
            ...
            {calendarModel.upcomingEvents.length === 0 && (
              <div className="p-4 text-sm text-slate-500">{loading ? 'Loading calendar' : 'No dated work'}</div>
            )}
          </div>
        </Panel>
